...
class TestE2ESchemaEvolution:
    ...
    def test_column_addition_compatibility(self, db_e2e, pggit_installed):
    ...
    def test_data_type_compatibility(self, db_e2e, pggit_installed):
        ...
        result = db_e2e.execute("SELECT * FROM public.datatype_test WHERE id = 1")
        assert result[0][1] == 42, "Integer should be stored correctly"
        assert result[0][4] is True, "Boolean should be stored correctly"

    def test_index_creation_and_usage(self, db_e2e, pggit_installed):
        ...
        # Create index
        db_e2e.execute("CREATE INDEX idx_indexed_col ON public.index_test(indexed_col)")
    ...
    def test_table_rename_compatibility(self, db_e2e, pggit_installed):
        ...
        # Rename table
        db_e2e.execute("ALTER TABLE public.original_name RENAME TO renamed_table")

        # Verify data is still accessible
        result = db_e2e.execute("SELECT value FROM public.renamed_table WHERE id = 1")
