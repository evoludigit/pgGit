...
class TestBasicDependencies:
    ...
    def test_view_depends_on_table(self, db_e2e, pggit_installed):
        """Test dependency between view and table."""
        db_e2e.execute("CREATE TABLE users (id INT, name TEXT, email TEXT)")
        db_e2e.execute("CREATE VIEW v_users AS SELECT id, name FROM users")

        # Verify view works
        db_e2e.execute("INSERT INTO users VALUES (1, 'Alice', 'alice@example.com')")
        result = db_e2e.execute("SELECT * FROM v_users")
        assert result, "View not working"
        assert result[0] == (1, 'Alice')

        # Get impact analysis for base table
        try:
            impact = db_e2e.execute("""
        ...
        # Cleanup
        db_e2e.execute("DROP VIEW v_users")
        db_e2e.execute("DROP TABLE users")
        print("✓ View-table dependency tracked")

    def test_foreign_key_dependency(self, db_e2e, pggit_installed):
        ...
        result = db_e2e.execute("SELECT COUNT(*) FROM child WHERE parent_id = %s", parent_id)
        assert result[0][0] == 1, "Child record not inserted"

        # Try to delete parent (should fail due to FK)
        with pytest.raises(Exception) as exc:
            db_e2e.execute("DELETE FROM parent WHERE id = %s", parent_id)
    ...
    def test_function_table_dependency(self, db_e2e, pggit_installed):
        ...
        db_e2e.execute("CREATE TABLE products (id INT, name TEXT, price DECIMAL)")
        db_e2e.execute("""
        ...
        # Insert test data
        db_e2e.execute("INSERT INTO products VALUES (1, 'Widget', 9.99)")

        # Test function
        result = db_e2e.execute("SELECT get_product_name(1)")
        assert result[0][0] == 'Widget', "Function not working"

        # Cleanup
        db_e2e.execute("DROP FUNCTION get_product_name")
        db_e2e.execute("DROP TABLE products")
        print("✓ Function-table dependency works")

    def test_index_table_dependency(self, db_e2e, pggit_installed):
        """Test index dependency on table."""
        db_e2e.execute("CREATE TABLE indexed_table (id INT, email TEXT)")
        db_e2e.execute("CREATE INDEX idx_email ON indexed_table(email)")
        ...
        db_e2e.execute("DROP TABLE indexed_table CASCADE")
...
class TestComplexDependencies:
    ...
    def test_dependency_chain(self, db_e2e, pggit_installed):
        """Test multi-level dependency chain."""
        # Create base table
        db_e2e.execute("CREATE TABLE t1 (id INT, data TEXT)")

        # Create views in chain: t1 → v1 → v2 → v3
        db_e2e.execute("CREATE VIEW v1 AS SELECT * FROM t1")
        db_e2e.execute("CREATE VIEW v2 AS SELECT * FROM v1")
        db_e2e.execute("CREATE VIEW v3 AS SELECT * FROM v2")

        # Insert test data
        db_e2e.execute("INSERT INTO t1 VALUES (1, 'test')")

        # Verify chain works
        result = db_e2e.execute("SELECT * FROM v3")
        assert result, "View chain broken"
        assert result[0] == (1, 'test'), "Data not flowing through chain"

        # Cleanup (should cascade through chain)
        db_e2e.execute("DROP VIEW v3")
        db_e2e.execute("DROP VIEW v2")
        db_e2e.execute("DROP VIEW v1")
        db_e2e.execute("DROP TABLE t1")
        print("✓ Multi-level dependency chain works")

    def test_dependency_on_drop_cascade(self, db_e2e, pggit_installed):
        """Test dropping object with dependencies using CASCADE."""
        db_e2e.execute("CREATE TABLE cascade_base (id INT, value TEXT)")
        db_e2e.execute("CREATE VIEW v_cascade AS SELECT * FROM cascade_base")

        # Verify both exist
        table_result = db_e2e.execute("""
            SELECT tablename FROM pg_tables
            WHERE tablename = 'cascade_base'
        """)
        view_result = db_e2e.execute("""
        ...
        db_e2e.execute("DROP TABLE cascade_base CASCADE")
    ...
    def test_self_referential_foreign_key(self, db_e2e, pggit_installed):
        ...
        root_id = db_e2e.execute_returning(
        ...
        # Cleanup
        db_e2e.execute("DROP TABLE categories CASCADE")
        print("✓ Self-referential FK works")

    def test_circular_view_prevention(self, db_e2e, pggit_installed):
        """Test that PostgreSQL prevents circular view dependencies."""
        db_e2e.execute("CREATE TABLE base (id INT)")
        db_e2e.execute("CREATE VIEW v1 AS SELECT * FROM base")
        ...
        result = db_e2e.execute("SELECT COUNT(*) FROM v2")
        assert result is not None, "View chain broken"

        # Cleanup
        db_e2e.execute("DROP VIEW v2")
        db_e2e.execute("DROP VIEW v1")
        db_e2e.execute("DROP TABLE base")
        print("✓ View dependencies validated")

    def test_multi_table_foreign_keys(self, db_e2e, pggit_installed):
        """Test multiple foreign key dependencies."""
        # Create tables with complex FK relationships
        db_e2e.execute("CREATE TABLE organizations (id SERIAL PRIMARY KEY, name TEXT)")
        db_e2e.execute("CREATE TABLE users (id SERIAL PRIMARY KEY, org_id INT REFERENCES organizations(id))")
        db_e2e.execute("""
        ...
        # Insert test data
        org_id = db_e2e.execute_returning(
            "INSERT INTO organizations (name) VALUES (%s) RETURNING id",
            "Test Org"
        )[0]

        user_id = db_e2e.execute_returning(
            "INSERT INTO users (org_id) VALUES (%s) RETURNING id",
            org_id
        )[0]

        project_id = db_e2e.execute_returning(
        ...
        # Cleanup
        db_e2e.execute("DROP TABLE projects")
        db_e2e.execute("DROP TABLE users")
        db_e2e.execute("DROP TABLE organizations")
...
class TestDependencyQueries:
    ...
    def test_query_table_dependencies(self, db_e2e, pggit_installed):
        """Test querying what depends on a table."""
        db_e2e.execute("CREATE TABLE dep_base (id INT)")
        db_e2e.execute("CREATE VIEW v_dep1 AS SELECT * FROM dep_base")
        db_e2e.execute("CREATE VIEW v_dep2 AS SELECT * FROM dep_base")
        ...
        # Cleanup
        db_e2e.execute("DROP VIEW v_dep1")
        db_e2e.execute("DROP VIEW v_dep2")
        db_e2e.execute("DROP TABLE dep_base")
        print("✓ Dependency queries work")

    def test_query_foreign_key_constraints(self, db_e2e, pggit_installed):
        """Test querying foreign key constraints."""
        db_e2e.execute("CREATE TABLE fk_parent (id SERIAL PRIMARY KEY)")
        db_e2e.execute("""
        ...
        # Cleanup
        db_e2e.execute("DROP TABLE fk_child")
        db_e2e.execute("DROP TABLE fk_parent")
        print("✓ FK constraint queries work")

    def test_dependency_impact_simulation(self, db_e2e, pggit_installed):
        """Test simulating impact of dropping an object."""
        db_e2e.execute("CREATE TABLE impact_base (id INT)")
        db_e2e.execute("CREATE VIEW v_impact1 AS SELECT * FROM impact_base")
        db_e2e.execute("CREATE VIEW v_impact2 AS SELECT * FROM v_impact1")
        ...
        dependent_views = db_e2e.execute("""
            SELECT viewname FROM pg_views
        ...
        # Cleanup
        db_e2e.execute("DROP VIEW v_impact2")
        db_e2e.execute("DROP VIEW v_impact1")
        db_e2e.execute("DROP TABLE impact_base")
