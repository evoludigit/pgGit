...
class TestE2EAdvancedBranching:
    ...
    def test_nested_branch_creation(self, db_e2e, pggit_installed):
        ...
        grandchild_id = db_e2e.execute_returning(
            "INSERT INTO pggit.branches (name) VALUES (%s) RETURNING id",
            "grandchild-branch"
        )[0]

        # Verify all branches exist
        main_exists = db_e2e.execute(
            "SELECT COUNT(*) FROM pggit.branches WHERE id = %s", parent_id
        )[0][0]
        child_exists = db_e2e.execute(
            "SELECT COUNT(*) FROM pggit.branches WHERE id = %s", child_id
        )[0][0]
        grandchild_exists = db_e2e.execute(
    ...
    def test_parallel_branch_operations(self, db_e2e, pggit_installed):
        """Test multiple branches can have independent operations"""
        branch1_id = db_e2e.execute_returning(
            "INSERT INTO pggit.branches (name) VALUES (%s) RETURNING id",
            "parallel-branch-1"
        )[0]

        branch2_id = db_e2e.execute_returning(
            "INSERT INTO pggit.branches (name) VALUES (%s) RETURNING id",
            "parallel-branch-2"
        )[0]
        ...
        branch1_commits = db_e2e.execute(
            "SELECT COUNT(*) FROM pggit.commits WHERE branch_id = %s", branch1_id
        )[0][0]
        branch2_commits = db_e2e.execute(
    ...
    def test_branch_cleanup_cascade(self, db_e2e, pggit_installed):
    ...
    def test_branch_retrieval_integrity(self, db_e2e, pggit_installed):
