...
```sql
-- Verify pggit_v2 is installed
SELECT pggit_v2.get_head_sha() as status;
...
-- Commit the changes
SELECT pggit_v2.create_basic_commit('Add users table');
```

### 3. Start Branching
```sql
-- Create a feature branch
SELECT pggit_v2.create_branch('feature/user-profiles', 'Add user profile features');
...
SELECT pggit_v2.create_basic_commit('Add user profile fields');
```
...
SELECT pggit_v2.create_branch('feature/new-api-endpoint', 'Add REST API for orders');
...
SELECT pggit_v2.create_basic_commit('Add API logging table for performance monitoring');

-- 4. Continue development
ALTER TABLE api_logs ADD COLUMN user_id INTEGER REFERENCES users(id);
SELECT pggit_v2.create_basic_commit('Link API logs to users for audit trail');
```

### Review: Branch Comparison
```sql
-- Compare your branch with main
SELECT * FROM pggit_v2.diff_branches('main', 'feature/new-api-endpoint');
...
SELECT pggit_v2.create_basic_commit('Merge feature/new-api-endpoint - API logging system');

-- Clean up branch
SELECT pggit_v2.delete_branch('feature/new-api-endpoint');
```
...
SELECT * FROM pggit_v2.get_object_history('public', 'users', 10);
...
```sql
-- For new features
SELECT pggit_v2.create_branch('feature/user-authentication');
-- Develop feature
-- Commit changes
-- Merge when complete
SELECT pggit_v2.delete_branch('feature/user-authentication');
```
...
-- Tag for production
SELECT pggit_v2.create_basic_commit('Release v2.1.0 - production ready');
```

### Hotfix Branches
```sql
-- For urgent production fixes
SELECT pggit_v2.create_branch('hotfix/security-patch');
-- Implement fix
SELECT pggit_v2.create_basic_commit('Fix critical security vulnerability');
-- Deploy immediately
```
...
SELECT pggit_v2.create_basic_commit('Add user authentication with JWT tokens');

-- Bad: Vague or unhelpful
SELECT pggit_v2.create_basic_commit('Changes');
```
...
SELECT name FROM pggit_v2.list_branches()
...
SELECT pggit_v2.create_basic_commit('Initial schema setup');
```

#### Branch Already Exists
```sql
-- Solution: Use unique branch names
SELECT pggit_v2.create_branch('feature/new-feature-v2');
```
...
psql -c "SELECT * FROM pggit_v2.validate_data_integrity()" > integrity_check.txt

# Schema diff for migration
psql -c "SELECT * FROM pggit_v2.diff_branches('staging', 'production')" > schema_diff.sql

# Generate deployment report
psql -c "SELECT * FROM pggit_v2.generate_monitoring_report()" > deployment_report.txt
```
...
def check_schema_health():
    with psycopg2.connect(database="mydb") as conn:
        with conn.cursor() as cur:
            # Check for alerts
            cur.execute("SELECT * FROM pggit_v2.check_for_alerts()")
            alerts = cur.fetchall()

            # Get dashboard summary
            cur.execute("SELECT * FROM pggit_v2.get_dashboard_summary()")
...
SELECT pggit_v2.create_basic_commit('Baseline schema - migrating from manual tracking');
...
-- 2. Create initial pggit_v2 commit
SELECT pggit_v2.create_basic_commit('Migrated from [old_tool] - initial schema state');
...
-- Reviewer workflow
-- 1. Check branch diff
SELECT * FROM pggit_v2.diff_branches('main', 'feature/new-table');
...
CREATE TRIGGER protect_main_branch
    BEFORE UPDATE ON pggit_v2.refs
