...
CREATE OR REPLACE VIEW pggit_v2.recent_commits_by_author AS
SELECT
    author,
...
JOIN pggit_v2.commit_graph ON commit_graph.commit_sha = changes.commit_sha
GROUP BY object_schema, object_name
ORDER BY change_count DESC;

COMMENT ON VIEW pggit_v2.most_changed_objects IS
...
CREATE OR REPLACE VIEW pggit_v2.branch_comparison AS
SELECT
...
COMMENT ON VIEW pggit_v2.branch_comparison IS
...
CREATE OR REPLACE VIEW pggit_v2.daily_change_summary AS
SELECT
...
COMMENT ON VIEW pggit_v2.schema_growth_history IS
...
CREATE OR REPLACE VIEW pggit_v2.author_activity AS
SELECT
    cg.author,
...
COMMENT ON VIEW pggit_v2.author_activity IS
...
CREATE OR REPLACE VIEW pggit_v2.commits_without_message AS
SELECT
    commit_sha,
    author,
    committed_at,
...
COMMENT ON VIEW pggit_v2.commits_without_message IS
...
CREATE OR REPLACE VIEW pggit_v2.orphaned_objects AS
SELECT DISTINCT
    o.sha,
    o.type,
    o.size,
...
FROM pggit_v2.objects o
LEFT JOIN pggit_v2.tree_entries te ON te.object_sha = o.sha
WHERE te.object_sha IS NULL
ORDER BY o.created_at DESC;

COMMENT ON VIEW pggit_v2.orphaned_objects IS
...
CREATE OR REPLACE VIEW pggit_v2.large_commits AS
SELECT
    cg.commit_sha,
    cg.author,
    cg.committed_at,
...
COMMENT ON VIEW pggit_v2.large_commits IS
...
CREATE OR REPLACE VIEW pggit_v2.current_head_info AS
SELECT
    ...
    (SELECT COUNT(*) FROM pggit_v2.tree_entries WHERE tree_sha = cg.tree_sha) as object_count
FROM pggit_v2.commit_graph cg
ORDER BY cg.committed_at DESC
LIMIT 1;

COMMENT ON VIEW pggit_v2.current_head_info IS
'Quick snapshot: current HEAD commit details and schema object count.';

-- View: Branch status summary
CREATE OR REPLACE VIEW pggit_v2.branch_status_summary AS
SELECT
    'Branches' as metric,
    COUNT(*)::TEXT as value
FROM pggit_v2.refs
...
COMMENT ON VIEW pggit_v2.branch_status_summary IS
...
SELECT
    ...
    (SELECT MAX(committed_at) FROM pggit_v2.commit_graph) as last_activity
FROM pggit_v2.commit_graph cg
LEFT JOIN pggit_audit.changes c ON c.commit_sha = cg.commit_sha;
