...
SELECT pggit_v2.delete_branch('uat-test-branch');
...
SELECT * FROM pggit_v2.get_object_history('uat_test', 'users', 5);

-- 8. diff_commits() - Compares commits
\echo '\n8. Testing diff_commits()...'
-- Get two commits to compare
SELECT commit_sha FROM pggit_v2.commit_graph ORDER BY committed_at DESC LIMIT 2;

-- 9. diff_branches() - Compares branches
\echo '\n9. Testing diff_branches()...'
-- Create two branches for comparison
SELECT pggit_v2.create_branch('uat-branch-a');
SELECT pggit_v2.create_branch('uat-branch-b');
SELECT * FROM pggit_v2.diff_branches('uat-branch-a', 'uat-branch-b');

-- 10. get_object_definition() - Gets DDL
\echo '\n10. Testing get_object_definition()...'
SELECT pggit_v2.get_object_definition('uat_test', 'users', 'TABLE');

-- 11. get_object_metadata() - Gets metadata
\echo '\n11. Testing get_object_metadata()...'
SELECT * FROM pggit_v2.get_object_metadata('uat_test', 'users', 'TABLE');
...
SELECT pggit_v2.delete_branch('uat-branch-a');
SELECT pggit_v2.delete_branch('uat-branch-b');
